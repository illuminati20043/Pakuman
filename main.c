...
int check_collision(struct Pakuman pakuman, struct Fantome fantome) {
	...
	int main(){
		...
		SDL_RenderClear(ren);
		 for (int i = 0; i < MAP_HEIGHT; i++) {
            ...
            if(fantome.mange==0){ Deplacer_Fantome(&fantome, win, ren, map);}
            if(fantome1.mange==0){ Deplacer_Fantome(&fantome1, win, ren, map);}
            if(fantome2.mange==0){ Deplacer_Fantome(&fantome2, win, ren, map);}
            if(fantome3.mange==0){ Deplacer_Fantome(&fantome3, win, ren, map);}
    ...
    if (pakuman.peutmanger == 0) {
        ...
        if (check_collision(pakuman, fantome3) && !fantome3.mange) {
    	    ...
    	    if(ghostEaten==1) {ghostEaten = 0;nbrmanges++;} 
